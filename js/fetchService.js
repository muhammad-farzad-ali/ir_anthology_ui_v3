...
function buildCountQuery(sparqlQuery) {
    const withoutSelect = sparqlQuery.replace(/SELECT[\s\S]*?WHERE/, 'SELECT (COUNT(DISTINCT ?entity_URI) AS ?count) WHERE');
    const withoutOrder = withoutSelect.replace(/ORDER BY[\s\S]*?(LIMIT|$)/m, '$1');
